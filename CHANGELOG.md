...
- Every file now names each band and labels each time step. `rumi.write`
  ...
  annual composite, takes a `(start, end)` interval. Band and time inputs must
  be ordered; sets are rejected, and iterators are consumed only up to one
  entry beyond the expected axis length.
...
- The C API adds `band_texts` and `band_text_count` to `rumi_write_desc` and
  `rumi_metadata`, and `time_type` no longer accepts 0. The writer checks
  individual and combined band-text sizes before copying, and time conversion
  checks its coordinate budget before allocating. `rumi_write_base_offset`
  uses only the layout, without accessing trailer metadata.

### Added

- `Metadata.bands` returns the band texts of a source. Metadata previews
  truncate long band texts and escape control characters.
